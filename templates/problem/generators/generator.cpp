...
void Generate_tests() {
    ...
    for (int tc = 1; tc <= T; ++tc) {
        // <<<PF_PLACEHOLDER>>>
        // Generate ONE test case here.
        // Example:
        //   int n = rnd.next(1, (int)min((ll)200000, sumNCap));
        //   sumNCap -= n;
        //   auto a = gen_arrays::random<ll>(n, -1000000000LL, 1000000000LL);
        //   cout << n << '
';
        //   gen_utils::print(a);
    }
...
// ── Generator usage in Polygon (place content in generators/script.txt) ──
// <#-- Executable name: generator -->
//
// Tests 1..N_samples  : hand-made (samples, uploaded by polygon_agent.py)
// Tests N_samples+1.. : produced by this script
//
// <<<PF_PLACEHOLDER>>>
// Replace the block below with your actual FreeMarker script.
// Seed (last positional arg) drives testlib randomness.
// Always hit edge cases AND max-constraint stress tests.
//
// Example (single-test, n up to MAX_N):
//   <#list 1..2 as s>
//       generator --n=1 ${s} > $
//   </#list>
//   <#list 3..10 as s>
...
//   </#list>
//   generator --n=200000 --maxval=1000000000 21 > $
//   generator --n=200000 --maxval=1000000000 22 > $
